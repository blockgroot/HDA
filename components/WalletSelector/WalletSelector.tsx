...
import { WalletStatus, ConnectType, useWallet, Connection } from "@terra-money/wallet-provider";
...
const WalletSelector = ({
    ...
    const installWallet = useCallback((type: ConnectType) => {
		wallet.install(type);
		setOpen(false);
	}, [wallet.connect]);

	const connectWallet = useCallback((type: Connection) => {
    ...
    if (wallet.wallets.length === 0 || wallet.status === WalletStatus.WALLET_NOT_CONNECTED) {
        return (
                        ...
                        <div className="dropdown-container">
                            <div className={`dropdown-box ${styles.walletDropdown}`}>
                                <div className={styles.walletContainer}>
                                    ...
                                    {wallet.availableConnections.filter(t => t.type != ConnectType.READONLY).map(type => (
                                        <button
