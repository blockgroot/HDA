...
function AssociateNow(props: AssociateNowProps) {
  ...
  const [isLoadCheckAssociated, setIsLoadCheckAssociated] = useState(false);
  ...
  }, [setIsShowLoadingModal, setIsLoadAssociate]);
    ...
    <>
      <Box className={"mb-10 p-0"}>
        <div className={styles.root}>
          {isLoadCheckAssociated && (
            <Loader text={"Please wait while we set things up for you"} />
          )}
          {!isLoadCheckAssociated && isAssociated && (
            <AssociateNowComplete onClickStartStaking={onClickStartStaking} />
          )}
          {!isLoadCheckAssociated && !isAssociated && (
      ...
      {showLoadingModal && (
        <Modal open={true} className="dialog">
          <Dialog
            ...
            onClose={isAssociated ? onCloseModalHandler : null}
          >
            {isLoadAssociate && (
              <Loader text={"Please wait while we set things up for you"} />
            )}
            {isAssociated && (
              <AssociateModalSuccess
                onClickStartStaking={onCloseModalHandler}
              />
            )}
          </Dialog>
        </Modal>
